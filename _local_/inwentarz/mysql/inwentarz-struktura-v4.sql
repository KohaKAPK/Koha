...
CREATE TABLE IF NOT EXISTS `invbook_definitions` ( -- inventory book: definitions
   ...
   `active` tinyint(4) default 1, --  is given book still in active use (1 for yes, 0 for no); if not, it would not be choosable/visible in various places
   ...
   `cn_prefix` varchar(30) default '', -- _call number_ prefix for items stored in this book (optional; used only for semi-automatic determinig/guessing to which book[s] given item may belong to), eg.: 'II-'
   `cn_suffix` varchar(30) default '', -- _call number_ suffix for items stored in this book (optional; used only for semi-automatic determinig/guessing to which book[s] given item may belong to), eg.: '/C'
   ...
   -- fields specific for book types 'W' (Writeoffs) and 'I' (Inventory/Items)
   `writeoff_parent_invbook_defn_id` int(11), -- inventory book <-> writeoff book association (by definition book id; circular 1:1 reference between two related 'I' & 'W' books)

   -- optional fields for book types 'A' (Accessions re/ number_*), 'W' (Writeoffs re/ base_document_number_*), 'I' (_inventory number_ prefix/suffix for given book, if any)
...
CREATE TABLE IF NOT EXISTS `invbook_permissions` ( -- inventory book: user permissions / access restriction
   ...
   CONSTRAINT `invbook_permissions_ibfk_1` FOREIGN KEY (`userid`) REFERENCES `borrowers` (`borrowernumber`) ON DELETE CASCADE ON UPDATE CASCADE,
   CONSTRAINT `invbook_permissions_ibfk_2` FOREIGN KEY (`invbook_definition_id`) REFERENCES `invbook_definitions` (`invbook_definition_id`) ON DELETE RESTRICT ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE IF NOT EXISTS `invbook_accessions` ( -- inventory book: accessions register
   ...
   `number_prefix` varchar(30) NOT NULL DEFAULT '', -- e.g. "WM-N-Z-", "2014/", authorized value (???)
   `number_cnt` int(11) NOT NULL, -- e.g. "123", auto-generated (???)
   `number_suffix` varchar(30) NOT NULL DEFAULT '', -- e.g. "/2014",  authorized / auto generated value (???)
   ...
   `volumes_count` int(10) NOT NULL DEFAULT 0, -- count of items on invoice
   `fascile_count` int(10) NOT NULL DEFAULT 0, -- count of items on invoice
   `special_count` int(10) NOT NULL DEFAULT 0, -- count of volumes on invoice, for special cases this may be different form volumes_count, since volumes count is count of items in inventory
   `other_count` varchar(255) NOT NULL DEFAULT 0, -- count of items on invoice
   `invoice_document_nr` varchar(255) NOT NULL DEFAULT '', -- invoice / document number (from acquisition module, or entered manually); CHECKME: 1|1+ ???
   `invoice_id` int(11), -- ??? foreign key, references aqinvoices->invoiceid; if NULL, invoice_document_nr is managed manually
   `vendor_name` mediumtext NOT NULL DEFAULT '', -- vendor name (copied from aqbooksellers->name, and/or entered manually ??)
   `vendor_id` int(11), -- ??? foreign key, references aqbooksellers->id
   `acquisition_mode` varchar(10) NOT NULL DEFAULT '', -- coded value (Z|K|W|D|I) determinig acquisition mode (if not empty, would serve as default value for acquisition_mode in invbook_items table)
   ...
   `date_entered` date, -- when this entry was (manually ?) added (YYYY-MM-DD)
...
CREATE TABLE IF NOT EXISTS `invbook_writeoffs` ( -- inventory book: writeoffs register
   ...
   `reason` varchar(255) NOT NULL, -- coded / authorized value (ZN|NZ|W|NO|I), reason why items were written off (or should this field rather be more granular / in invbook_writeoff_bases ???)
   `current_status` varchar(30) NOT NULL default 'NA', -- given writeoff entry & accompanying document current status; coded value (NA|C|CN|CNP, i.e.: New-Active|Closed|Closed-Numbered|Closed-Numbered-Printed)
...
CREATE TABLE IF NOT EXISTS `invbook_items` ( -- inventory book: item register
   ...
   `publication_nr` varchar(255) default '',
   ...
   `tmp_import_title_hash` varchar(255) default '', -- temporary field used to [re]associate inventory book item title with bibio record after import
   `tmp_import_barcode_hash` varchar(255) default '', -- temporary field used to [re]associate inventory book item with item record after import
...
CREATE TABLE IF NOT EXISTS `invbook_writeoff_bases` ( -- inventory book: writeoff bases / accompanying documents individual item entries
   ...
   `invbook_definition_id` int(11) NOT NULL, -- foreign key: book definition id (type 'W')
   `writeoff_id` int(11) NOT NULL, -- foreign key: references writeoff_id from invbook_writeoffs
   `invbook_item_id` int(11) NOT NULL, -- references invbook_items (non-foreign key to avoid circular reference)

   `seq_number` int(11) default NULL, -- sequential number of this entry/item as appearing on printed form of writeoffs bases / accompanying documents
   ...
   UNIQUE KEY `wob_seq_number_pseudo_unique_key` (`invbook_definition_id`,`seq_number`),
   UNIQUE KEY `wob_invbook_item_id_pseudo_unique_key` (`invbook_definition_id`,`invbook_item_id`),
   KEY `invbook_definition_id` (`invbook_definition_id`),
